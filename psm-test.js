...
const app = {
    ...
    attachEventListeners() {
        ...
        if (startIndexInput) {
            startIndexInput.addEventListener('change', () => this.validateStartIndex());
        }

        // Question count input change
        const questionCountInput = document.getElementById('questionCountInput');
        if (questionCountInput) {
            questionCountInput.addEventListener('change', () => this.updateQuestionCount());
        }
        ...
        if (dashboardView) {
            ...
            if (btns[1]) btns[1].addEventListener('click', () => this.showStatsView());
        }
        ...
        if (resultsView) {
            ...
            if (btns[0]) btns[0].addEventListener('click', () => this.showReviewView());
            if (btns[1]) btns[1].addEventListener('click', () => this.showDashboard());
        }
        ...
        if (reviewView) {
            ...
            if (btns[0]) btns[0].addEventListener('click', () => this.showResultsView());
        }
    ...
    startNewTest() {
        ...
        if (this.testMode === 'random') {
            ...
            const endIdx = Math.min(startIdx + this.questionCount, this.allQuestions.length);
            ...
            if (selectedQuestions.length < this.questionCount) {
                ...
                alert(`Only ${actualCount} questions available from index ${this.startQuestionIndex}. Test will have ${actualCount} questions instead of ${this.questionCount}.`);
            }
        }
    ...
    showStatsView() {
        ...
        if (stats.wrongAnswers.length === 0) {
            wrongAnswersHTML = '<p class="text-center text-success mt-3">🎉 No wrong answers in this period!</p>';
        } else {
            wrongAnswersHTML = `
                <div class="mt-4">
